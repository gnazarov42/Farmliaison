const PLACE_DETAILS_FIELDS =
  'place_id,name,rating,geometry,formatted_phone_number,photos,reviews,address_components,icon,price_level,website,adr_address,formatted_address,opening_hours,price_level';
...
export default defineEventHandler(async (event) => {
  ...
  const keyDetails = await fetch(
    `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=${PLACE_DETAILS_FIELDS}&key=${process.env.GOOGLE_API_KEY}&maxResults=10`,
    {
      method: 'GET',
    },
  );
  ...
  if (coordinates) {
    ...
    // console.log('🚀 ~ defineEventHandler ~ coordinates:', coordinates);
    // console.log('🚀 ~ defineEventHandler ~ fetchedCoords:', fetchedCoords);
    if (
