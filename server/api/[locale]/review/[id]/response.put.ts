import { updateResponseToReview } from '~/server/db/review';
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const updatedReview = await updateResponseToReview(
      reviewId,
    ...
    return {
      statusCode: 200,
      body: { message: 'Response updated successfully', review: updatedReview },
    };
  } catch (error: any) {
    return sendError(
      event,
